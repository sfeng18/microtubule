...
{ //求符合标准正态分布的随机变量 X<x 的概率，精确到 1e-9
    int i = 0;
...
void RotMatrx(double *Mrot, double *Qin)
{ //依据四元数求旋转矩阵
...
void CoodRot(double *Rout, double *Rin, double *Mrot)
{ //依据旋转矩阵转动坐标
    double r[3];
    r[0] = Rin[0];
    r[1] = Rin[1];
...
void CrossL(double *a, double *b, double *c)
{ //c=a X b
    c[0] = a[1] * b[2] - a[2] * b[1];
    c[1] = a[2] * b[0] - a[0] * b[2];
    c[2] = a[0] * b[1] - a[1] * b[0];
...
void QuaTimes(double *a, double *b, double *c)
{ //四元数乘法  c=a * b
    int i;
...
void Set_Tub(tubulin *Tubin, double *r, double *quaternion, double xi)
{ //,double a,double b,double c){    //粒子状态设定
    int j = 0, MyHT;
...
void Set_FileName()
{
    ...
    if (HELIX_ON)
        ...
        HelType = 'I';
    if (Whole_Struc == 1)
        NameL = sprintf(fstring, "%c%dX%d_R%g-%gB%g-%gT%gG%g_L%gA%g_N%d%c%d",
        ...
        NameL = sprintf(fstring, "F%dX%d_R%g-%gB%g-%gT%gG%g_L%gA%g_N%d%c%d",
                        MTimes, Times, 1.0 * P_LJC, 1.0 * P_LJS, 1.0 * KLong, 1.0 * KLat, 1.0 * TLong, 1.0 * GLat, 1.0 * LCutEd, ACutEd * 180 / pi, N, HelType, LCompute);
    else
        NameL = sprintf(fstring, "%c%dX%d_R%g-%gB%g-%gT%gG%g_L%gA%g_N%d.%d",
    ...
    if (TwistType == 2)
        NameL += sprintf(fstring + NameL, "_TT");
    NameL += sprintf(fstring + NameL, "_EL%g", 1.0 * ELatAA);
    {
        ...
        if (typel || typer)
        {
            ...
            if (typel)
                NameL += sprintf(fstring + NameL, "l%c", typel);
            if (typer)
                NameL += sprintf(fstring + NameL, "r%c", typer);
        }
    }
    if (ForceXYZ & 0xF0)
    {
    ...
    if (DeltaL != 0.0 || DeltaXi != 0.0 || DeltaA != 0.0)
    {
        ...
        if (DeltaL != 0.0)
            NameL += sprintf(fstring + NameL, "L%g", 1.0 * DeltaL);
        if (DeltaXi != 0.0)
            NameL += sprintf(fstring + NameL, "X%g", 1.0 * DeltaXi);
        if (DeltaA != 0.0)
            NameL += sprintf(fstring + NameL, "A%g", 1.0 * DeltaA);
    }
    if (CAP_ON)
        NameL += sprintf(fstring + NameL, "_Cap%d", CAP_ON);
    if (FR_ON || MR_ON)
        NameL += sprintf(fstring + NameL, "_T%g", 1.0 * (MD_ON ? TReal : KT));
    if (MD_ON)
    {
        if (FR_ON)
            NameL += sprintf(fstring + NameL, "_Fr");
        if (MR_ON)
            NameL += sprintf(fstring + NameL, "_Mr");
    }
    else
        NameL += sprintf(fstring + NameL, "_R%gA%g", 1.0 * StepR, 1.0 * StepA);
    if (W_Out != 1)
        NameL += sprintf(fstring + NameL, "_Out%d", W_Out);
    if (W_Lat_aa != 0.1811)
        NameL += sprintf(fstring + NameL, "_WL%g_%g", 1.0 * W_Lat_aa, 1.0 * W_Lat_ab);
    if (Test_Type)
        NameL += sprintf(fstring + NameL, "_%c%dX%g", Test_Type, SecSteps, 1.0 * SecLen);
    if (Test_Type == 'Y' || Test_Type == 'K')
        NameL += sprintf(fstring + NameL, ".R%d", BallR);
    if (Test_Type == 'T')
        NameL += sprintf(fstring + NameL, ".F%dL%d", CAPBegin, CAPNo);
    if (XI_0 != 5.0)
        NameL += sprintf(fstring + NameL, "_xi%g", 1.0 * XI_0);
    if (PR_ON)
        NameL += sprintf(fstring + NameL, "_Pr%d", PR_ON);
    if (GROW_ON || BREAK_ON || HD_ON)
    {
        NameL += sprintf(fstring + NameL, "_");
        if (GROW_ON)
            NameL += sprintf(fstring + NameL, "G");
        if (GROW_ON != 1)
            NameL += sprintf(fstring + NameL, "%d", GROW_ON);
        if (BREAK_ON)
            NameL += sprintf(fstring + NameL, "B");
...
#endif
        if (GROW_ON && G_Slow)
            NameL += sprintf(fstring + NameL, "S%d", G_Slow);
        if (BREAK_ON)
            NameL += sprintf(fstring + NameL, "C%g.%dW", 1.0 * C_rate, CTime / 10000);
        if (BREAK_ON)
            NameL += sprintf(fstring + NameL, "S%gB%g", 1.0 * S_rate, 1.0 * B_rate);
#if MustGrow
        NameL += sprintf(fstring + NameL, "F");
#endif
        if (HD_ON == 1)
            NameL += sprintf(fstring + NameL, "HD%g", 1.0 * HD_rate);
        else if (HD_ON == 2)
            NameL += sprintf(fstring + NameL, "HDD%g", 1.0 * Afree); //dynamic
        if (HD_ON && HD_Delay)
            NameL += sprintf(fstring + NameL, "D%dk", HD_Delay / 1000);
    }
    else if (LongRFTest)
        NameL += sprintf(fstring + NameL, "_HD%g", 1.0 * HD_rate);
    if (GRStep)
        NameL += sprintf(fstring + NameL, "_Gr%d", GRStep);
    if (RLW_ON)
    {
        NameL += sprintf(fstring + NameL, "_RLW%gN%d", 1.0 * RandLatP, GammaN);
        if (RLW_ON < 10)
            NameL += sprintf(fstring + NameL, "N");
        if (RLW_ON >= 10)
            NameL += sprintf(fstring + NameL, "Z");
        if (RLW_ON >= 15 && RLW_ON < 20)
            NameL += sprintf(fstring + NameL, "H1");
        else if (RLW_ON >= 20 && RLW_ON < 25)
            NameL += sprintf(fstring + NameL, "H2");
        else if (RLW_ON >= 25 && RLW_ON < 30)
            NameL += sprintf(fstring + NameL, "H3");
        else if (RLW_ON >= 30 && RLW_ON < 35)
            NameL += sprintf(fstring + NameL, "H4");
    }
    if (EOut_ON)
        NameL += sprintf(fstring + NameL, "_EO");
    if (RS_OUT_ON)
        NameL += sprintf(fstring + NameL, "_RS");
    if (FilaSet != 13)
        NameL += sprintf(fstring + NameL, "_IF%d", FilaSet);
    if (FilaMis != 0)
        NameL += sprintf(fstring + NameL, "M%d", FilaMis);
    if (TipPf != FilaSet)
        NameL += sprintf(fstring + NameL, "_%dTPF%dL%d", TipPfs, TipPf, TipLayer);
    if (TipSinA > 0)
    {
        if (TipType == 1)
            NameL += sprintf(fstring + NameL, "_TM");
        else if (TipType == 2)
            NameL += sprintf(fstring + NameL, "_TP");
        else if (TipType == 3)
            NameL += sprintf(fstring + NameL, "_TPS");
        else if (TipType == 4)
            NameL += sprintf(fstring + NameL, "_TD");
        else if (TipType == 5)
            NameL += sprintf(fstring + NameL, "_TL");
        else if (TipType == 6)
            NameL += sprintf(fstring + NameL, "_TAD");
        else
            NameL += sprintf(fstring + NameL, "_T");
        NameL += sprintf(fstring + NameL, "A%dL%g", TipSinA, 1.0 * TipSinL);
        if (TipType == 4 && TipARatio > 0)
            NameL += sprintf(fstring + NameL, "R%g", 1.0 * TipARatio);
        if ((TipType == 1 || TipType == 3 || TipType == 5 || TipType == 6) && TipSinPhase != 0)
            NameL += sprintf(fstring + NameL, "t%g", 1.0 * TipSinPhase);
        if (TipSinH > 0)
            NameL += sprintf(fstring + NameL, "H%d", TipSinH);
        if (PhaseFile > 0)
            NameL += sprintf(fstring + NameL, "F%d", PhaseFile);
    }
    if (RFLayers && RFUnits)
        NameL += sprintf(fstring + NameL, "_%dRF%dL%d", RFType, RFUnits, RFLayers);
    if (RFRate)
        NameL += sprintf(fstring + NameL, "_RFR%g", 1.0 * RFRate);
    if (TipEnd)
        NameL += sprintf(fstring + NameL, "_T%dA%dP%d", TipEnd, TipAll, TipEndPf);
    if (LongRFTest)
        NameL += sprintf(fstring + NameL, "_LRFT%d", LongRFTest);

    if (QDDelta)
        NameL += sprintf(fstring + NameL, "_Q%gT%0.2X", 1.0 * QDDelta, QDType);
    if (FixLink)
    {
        NameL += sprintf(fstring + NameL, "_fl");
        if (cutoff_s < cutoff)
            NameL += sprintf(fstring + NameL, "%g", cutoff_s);
    }
    if (OutputTime)
    {
        tnow = time(0);
...
void Set_NeighbourList(int InitMark)
{
    ...
    if (!InitMark)
        NbrList = (int *)malloc(Max_Nbr_Num * N * sizeof(int));
    for (i = 0; i < N; i++)
    {
...
void Set_QDRate(void)
{
    ...
    if (QDDelta)
    {
        ...
        for (i = 0; i < 10001; i++)
        {
            Dtmp = 0.001 * i;
            if (i)
                MyQD[i] = exp(QDk * log(QDk) + (QDk - 1) * log(Dtmp) - QDk * Dtmp - lgamma(QDk));
            else
                MyQD[i] = 0;
            if (i)
                QDList[i] = QDList[i - 1] + 0.001 * (MyQD[i] + MyQD[i - 1]) / 2;
        }
        QDList[10000 + 2] = 1;
    }
    else
        ...
        if (QDType & 0xF0)
        {
            ...
            else
            {
                for (j = 0; j < 2; j++)
                {
                    ...
                    {
                    case 0x20:
                        Tubp->QDRate[j] == 1.0 + QDDelta *(2 * R1 - 1);
                        break;
                    case 0x10:
                    {
                        p0 = 0;
                        ...
                        if (p0 != p2)
                            Tubp->QDRate[j] = 0.001 * p0 + 0.001 * (p2 - p0) * (R1 - QDList[p0]) / (QDList[p2] - QDList[p0]);
                        else
                            Tubp->QDRate[j] = 0.001 * p1;
                        break;
                    }
                    }
                }
                if (QDType & 0x40)
                    Tubp->QDRate[1] = Tubp->QDRate[0];
            }
        }
        if (QDType & 0x0F)
        {
...
int Set_Position(int i, double *r, double *angle, int FilaMisSt, int FilaMisEd)
{
    ...
    if (Whole_Struc == 1)
    {
        ...
        {
            if (HELIX_ON)
                r[0] = (i / FilaSet) * 4.0 + (i % FilaSet) * 6.0 / FilaSet - BoxLX + 5;
            else
                r[0] = (i / FilaSet) * 4.0 - BoxLX + 5;
            ...
            else
            {
                if (!TipType)
                {
                    ...
                    if ((i - NInit) / FilaSet + 0.5 < MyHeight)
                        return 1;
                    else
                        return 0;
                }
                ...
                else if (TipType == 4)
                {
                    ...
                    MyPos = i % FilaSet;
                    if (MyPos < 0.5 * FilaSet)
                        PeakNo = MyPos;
                    else if (FilaSet - MyPos < 0.5 * FilaSet)
                        PeakNo = FilaSet - MyPos;
                    if (PeakNo < 0.5 * PeakL)
                    {
                        PeakX = PeakNo;
        ...
        else if (FilaMisSt > 0)
        {
            ...
            {
                if (HELIX_ON)
                    r[0] = (i / 13) * 4.0 + (i % 13) * 6.0 / 13 - BoxLX + 5;
                else
                    r[0] = (i / 13) * 4.0 - BoxLX + 5;
            ...
            else if (i >= FilaMisSt && i < FilaMisEd)
            {
                ...
                if (HELIX_ON)
                    r[0] = k * 4.0 + l * 6.0 / FilaSet - BoxLX + 5;
                else
                    r[0] = k * 4.0 - BoxLX + 5;
...
void move(double *r, double *angle, double *xi)
{ //随机移动
    int No;
...
void move2(double *r, double *angle, double StepLen, int k)
{ //随机移动
    int No1, No2, No3;
...
double potential(tubulin *Tub_i, tubulin *Tub_j, double *PotenAll)
{ //粒子间能量计算
    int k, l;
    int No = 0, DNo[6];
    ...
    if (!Tub_i->Active || !Tub_j->Active)
        return 0;
    if (Tub_i->Tub_No == Tub_j->Tub_No)
        return 0;
    {
        for (l = 0; l < 3; l++)
        {
        ...
        if (rmod[0] <= cutoff)
        {
            ...
            for (k = 0; k < 4; k++)
                ADcy[k] = (1 - Delta[k]) * ansADcyC[DNo[k]] + (Delta[k]) * ansADcyC[DNo[k] + 1];
            ...
            if (RDcy[0] > 0 && (AD01 > 0 || AD23 > 0))
            {
                ...
                for (k = 0; k < 6; k++)
                    Theta[k] = (1 - Delta[k]) * ansACos[DNo[k]] + (Delta[k]) * ansACos[DNo[k] + 1];
                if (Dot(Tub_j->t, Pi) < 0)
                    Theta[2] = -Theta[2];
                if (Dot(Tub_i->t, Pj) < 0)
                    Theta[5] = -Theta[5];
            ...
            {
                if (Tub_i->right == Tub_j->Tub_No)
                    Tub_i->right = -1;
                if (Tub_j->left == Tub_i->Tub_No)
                    Tub_j->left = -1;
            }
            ...
            {
                if (Tub_i->left == Tub_j->Tub_No)
                    Tub_i->left = -1;
                if (Tub_j->right == Tub_i->Tub_No)
                    Tub_j->right = -1;
            }
        }
        if (rmod[1] <= cutoff)
        {
            ...
            if (Pr[1] < 0)
                Poten[0] += Pr[1] * ELong * AD45;
            else
                Poten[0] += Pr[1] * ELong;
            if (PrS[1] < 0)
                Poten[3] += PrS[1] * ELatBA * (AD01 + AD23);
            else
                Poten[3] += PrS[1] * ELatBA;
            ...
            {
                if (Tub_i->rseam == Tub_j->Tub_No)
                    Tub_i->rseam = -1;
                if (Tub_j->lseam == Tub_i->Tub_No)
                    Tub_j->lseam = -1;
            }
#endif
        }
        if (rmod[2] <= cutoff)
        {
            ...
            if (PrS[2] < 0)
                Poten[9] += PrS[2] * ELatBA * (AD01 + AD23);
            else
                Poten[9] += PrS[2] * ELatBA;
            if (Pr[2] < 0)
                Poten[6] += Pr[2] * ELong * AD45;
            else
                Poten[6] += Pr[2] * ELong;
            ...
            {
                if (Tub_i->lseam == Tub_j->Tub_No)
                    Tub_i->lseam = -1;
                if (Tub_j->rseam == Tub_i->Tub_No)
                    Tub_j->rseam = -1;
            }
#endif
        }
...
double poten_fl(tubulin *Tub_i, double *PotenAll)
{ //粒子间能量计算
    char MyMark;
    int i, j, k, l;
    int No = 0, DNo[6];
    ...
    for (j = 0; j < Tub_i->AddNum; j++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    if (PrS < 0)
                        PotenAll[j] += PrS * ELatBA * MyAD;
                    else
                        PotenAll[j] += PrS * ELatBA;
                }
                else
                {
                    if (PrS < 0)
                        PotenAll[j] += PrS * ELatAA * MyAD;
                    else
                        PotenAll[j] += PrS * ELatAA;
                }
            }
...
int Force(tubulin *Tubp_i, tubulin *Tubp_j)
{
    ...
    if (Tubp_i->Active == 0 || Tubp_j->Active == 0)
        return 0;
    ...
    for (k = 0; k < 3; k++)
    {
        ...
        {
            if (fabs(nangle) < 1)
                force_moment_n[k] += -2 * P_LT * acna / sin(acna) * lj[4] * Tubp_i->n[k];
            else
                force_moment_n[k] += -2 * P_LT * lj[4] * Tubp_i->n[k];
        }
    }
    for (k = 0; k < 3; k++)
    {
        ...
        if (gpair[4] > 0.5 || gpair[5] > 0.5)
            force_all[k] += 2 * P_LT * acna * lj[4] * acna * lj[4] * r[4][k];
    }
    moment[0] =
        -(r[0][1] * force_moment0[2] - r[0][2] * force_moment0[1]) - (r[1][1] * force_moment1[2] - r[1][2] * force_moment1[1]) - (r[2][1] * force_moment2[2] - r[2][2] * force_moment2[1]) - (r[3][1] * force_moment3[2] - r[3][2] * force_moment3[1]) + (Tubp_j->t[1] * force_moment_t[2] - Tubp_j->t[2] * force_moment_t[1]) + (Tubp_j->n[1] * force_moment_n[2] - Tubp_j->n[2] * force_moment_n[1]) + (Tubp_i->rbar1[1] * force_1[2] - Tubp_i->rbar1[2] * force_1[1]) + (Tubp_i->rbar2[1] * force_2[2] - Tubp_i->rbar2[2] * force_2[1]);
    moment[1] =
        -(r[0][2] * force_moment0[0] - r[0][0] * force_moment0[2]) - (r[1][2] * force_moment1[0] - r[1][0] * force_moment1[2]) - (r[2][2] * force_moment2[0] - r[2][0] * force_moment2[2]) - (r[3][2] * force_moment3[0] - r[3][0] * force_moment3[2]) + (Tubp_j->t[2] * force_moment_t[0] - Tubp_j->t[0] * force_moment_t[2]) + (Tubp_j->n[2] * force_moment_n[0] - Tubp_j->n[0] * force_moment_n[2]) + (Tubp_i->rbar1[2] * force_1[0] - Tubp_i->rbar1[0] * force_1[2]) + (Tubp_i->rbar2[2] * force_2[0] - Tubp_i->rbar2[0] * force_2[2]);
    moment[2] =
        -(r[0][0] * force_moment0[1] - r[0][1] * force_moment0[0]) - (r[1][0] * force_moment1[1] - r[1][1] * force_moment1[0]) - (r[2][0] * force_moment2[1] - r[2][1] * force_moment2[0]) - (r[3][0] * force_moment3[1] - r[3][1] * force_moment3[0]) + (Tubp_j->t[0] * force_moment_t[1] - Tubp_j->t[1] * force_moment_t[0]) + (Tubp_j->n[0] * force_moment_n[1] - Tubp_j->n[1] * force_moment_n[0]) + (Tubp_i->rbar1[0] * force_1[1] - Tubp_i->rbar1[1] * force_1[0]) + (Tubp_i->rbar2[0] * force_2[1] - Tubp_i->rbar2[1] * force_2[0]);
    ...
    if (Tubp_i->Tub_No < Tubp_j->Tub_No && Tubp_i->Tin % 100 == 0)
    {
        ...
        {
            if (Tubp_i->left == Tubp_j->Tub_No)
                Tubp_i->left = -1;
            if (Tubp_j->right == Tubp_i->Tub_No)
                Tubp_j->right = -1;
        }
        ...
        {
            if (Tubp_i->right == Tubp_j->Tub_No)
                Tubp_i->right = -1;
            if (Tubp_j->left == Tubp_i->Tub_No)
                Tubp_j->left = -1;
        }
#if HELIX_ON
        ...
        {
            if (Tubp_i->lseam == Tubp_j->Tub_No)
                Tubp_i->lseam = -1;
            if (Tubp_j->rseam == Tubp_i->Tub_No)
                Tubp_j->rseam = -1;
        }
        ...
        {
            if (Tubp_i->rseam == Tubp_j->Tub_No)
                Tubp_i->rseam = -1;
            if (Tubp_j->lseam == Tubp_i->Tub_No)
                Tubp_j->lseam = -1;
        }
#endif
    }
    if (Tubp_i->front == Tubp_j->Tub_No)
    {
        if ((Tubp_i->chain1[0] + r[1][0]) * Tubp_i->s[0] + (Tubp_i->chain1[1] + r[1][1]) * Tubp_i->s[1] + (Tubp_i->chain1[2] + r[1][2]) * Tubp_i->s[2] < 0)
        {
            ...
            Tubp_i->potenf = (-lj[1] * gpair[4] / 2 - 0.5);
    }
    else if (Tubp_i->back == Tubp_j->Tub_No)
    {
        if ((Tubp_i->chain2[0] + r[2][0]) * Tubp_i->s[0] + (Tubp_i->chain2[1] + r[2][1]) * Tubp_i->s[1] + (Tubp_i->chain2[2] + r[2][2]) * Tubp_i->s[2] > 0)
        {
            ...
            Tubp_i->potenb = (-lj[2] * gpair[5] / 2 - 0.5);
    }
...
{ //初始化
    ...
    if (Whole_Struc == 1 && TipSinA > 0)
    {
        ...
        if (TipType == 1 || TipType == 5)
        {
            ...
            for (i = 0; i < 1001; i++)
            {
                ...
                //                MySin=sin(L1*Dtmp);
                if (TipType == 1)
                    MySin = sqrt(L2 - TipSinL) * sin(L1 * Dtmp) + sqrt(TipSinL - L1) * sin(L2 * Dtmp + TipSinPhase * pi);
                //                else if(TipType==2)MySin=sin(Dtmp-pi/4);
                //                else if(TipType==3)MySin=sin(Dtmp+TipSinPhase*pi);
                else if (TipType == 5)
                    MySin = (L2 - TipSinL) * sin(L1 * Dtmp) + (TipSinL - L1) * sin(L2 * Dtmp + TipSinPhase * pi);
                if (MySin > MaxA)
                    MaxA = MySin;
                if (MySin < MinA)
                    MinA = MySin;
...
#elif FixLink
    {
        ...
        for (i = 0; i < N; i++)
        {
            ...
            for (j = 0; j < Tubp->AddNum; j++)
                fprintf(stderr, "\t%d", Tubp->LinkList[j]);
            fprintf(stderr, "\nLinkMark:");
            for (j = 0; j < Tubp->AddNum; j++)
                fprintf(stderr, "\t%0.2X", Tubp->PPMark[j]);
            fprintf(stderr, "\nLinkPoten:");
            for (j = 0; j < Tubp->AddNum; j++)
                fprintf(stderr, "\t%g", Tubp->PotenList[j]);
            fprintf(stderr, "\n");
        }
    }
#else
    ...
    if (myid == 0)
    {
        ...
        if (Tub[p].back > 0)
        {
            ...
            for (j = 0; j < topno; j++)
            {
                if (Tmark[j])
                {
                    ...
                    {
                        fprintf(stderr, "LinkTub:");
                        for (q = 0; q < Tubp->AddNum; q++)
                            fprintf(stderr, "\t%d", Tubp->LinkList[q]);
                        fprintf(stderr, "\nLinkMark:");
                        for (q = 0; q < Tubp->AddNum; q++)
                            fprintf(stderr, "\t%0.2X", Tubp->PPMark[q]);
                        fprintf(stderr, "\nLinkPoten:");
                        for (q = 0; q < Tubp->AddNum; q++)
                            fprintf(stderr, "\t%g", Tubp->PotenList[q]);
                        fprintf(stderr, "\n");
                    }
#endif
                    N += 1;
        ...
        else
        {
            ...
            if (length > 0)
            {
                ...
                if (Tubp->front >= 0)
                {
                    Tub[Tubp->front].back = -1;
                    Tubp->front = -1;
                }
                if (Tubp->back >= 0)
                {
                    Tub[Tubp->back].front = -1;
                    Tubp->back = -1;
                }
                if (Tubp->left >= 0)
                {
                    Tub[Tubp->left].right = -1;
                    Tubp->left = -1;
                }
                if (Tubp->right >= 0)
                {
                    Tub[Tubp->right].left = -1;
                    Tubp->right = -1;
                }
            }
...
void Monte_Carlo(int M)
{
    ...
    for (k = 0; k < Times; k++)
    {
        for (l = 0; l < N; l++)
        {
            ...
            Tubp->poten = 0;
            for (j = 0; j < Tubp->AddNum; j++)
                Tubp->poten += PotenTmp[j];

            if (KT > 1)
                move2(dr, dangle, 3.0, l % 2);
            else
                move2(dr, dangle, 1.0, l % 2);
        ...
        if (k % 20 == 10)
        {
            ...
            for (i = 0; i < N; i++)
            {
                ...
                {
#if HD_ON == 1
                    if (Tubp->front >= 0 && Tubp->Thd < Tubp->Tin)
                        Tubp->Type = 1;
#elif HD_ON == 2
                    double ActiveE, Cnow;
                    ActiveE = 0;
                    if (Tubp->front >= 0)
                        ActiveE += 0.161 + Tubp->potenf;
                    if (Tubp->back >= 0)
                        ActiveE += 0.161 + Tubp->potenb;
        ...
        for (l = 0; l < N; l++)
        {
            ...
            if (poten > Tubp->poten)
            {
                ...
                if (exp(dv) < ra)
                {
                    ...
                    for (j = 0; j < 12; j++)
                        Tub[i].MyPoten[j] = Tubp->MyPoten[j] / 2;
...
void MoleDynamic(int MoleDynamicNo)
{
    ...
    for (t = 0; t < Times; t++)
    {
        ...
        for (m = NStart + myid * MpiSec; m < NStart + (myid + 1) * MpiSec && m < N; m++)
        {
            ...
            if (!Tubp->Active)
                continue;
            if (Whole_Struc && Tubp->LineNo <= LStart)
                continue;
        ...
        //        if(1)
        {
            ...
            for (m = NStart + myid * MpiSec; m < NStart + (myid + 1) * MpiSec && m < N; m++)
            {
                ...
                for (p = 0; p < 3; p++)
                {
                    ...
                    if (m == 0)
                        Tubp->aomi[p] = Tubp->vomi[p] = Tubp->a[p] = Tubp->v[p] = 0;
#endif
...
//                        if(MoleDynamicNo<2)Tubp->a[p]=Tubp->v[p]=0;
            ...
            if (numprocs != 1)
            {
                ...
                if (myid != 0)
                {
                    if (myid != numprocs - 1)
                        ...
                        BcastSize = N - NStart - myid * MpiSec;
...
#endif
                if (!MConver && fabs(ETot[Trec - 1] - ETot[Trec]) < E_Conver)
                    TConver++;
                else if (!MConver)
                    TConver = 0;
                ...
                for (i = 0; i < N; i++)
                {
                    ...
                    if (myid == 0)
                    {
                        ...
                        if (Tubp->Type == 1)
                        {
                            ...
                            ra = (rand() / (RAND_MAX + 0.1));
                            if (Test_Type == 'W' && MaxEnd < 17)
                                VirtualL = 17 - MaxEnd;
                            else if (!Tubp->Type)
                                VirtualL = (Tubp->ChainNo <= 5) ? -Tubp->ChainNo : -5;
                            else
                                VirtualL = 0;
                            if (Tubp->Tub_No > 25 && Tubp->WallMark < -SlowTime && (Tubp->left < 0 || Tubp->right < 0) && exp(-B_rate * (Tubp->ChainNo + VirtualL) * (Tubp->ChainNo + VirtualL) * (Tubp->poten + 1.184) / 225) < ra)
                            {
                                ...
                                for (j = i, p = 0; j >= 0; j = Tub[j].front, p++)
                                {
                                    ...
                                    else
                                    {
                                        if (Tub[j].r[1] > Ymax)
                                            Ymax = Tub[j].r[1];
                                        else if (Tub[j].r[1] < Ymin)
                                            Ymin = Tub[j].r[1];
                                        if (Tub[j].r[2] > Zmax)
                                            Zmax = Tub[j].r[2];
                                        else if (Tub[j].r[2] < Zmin)
                                            Zmin = Tub[j].r[2];
                                    }
                                }
            ...
            if (myid == 0)
            {
                ...
                if (t % SlowTime == 0)
                {
                    for (i = 0; i < N; i++)
                    {
                        ...
                        if (Tubp->angle[3] > pi)
                            Tubp->angle[3] = Tubp->angle[3] - 2 * pi * ((int)(Tubp->angle[3] / pi / 2));
                        else if (Tubp->angle[3] < -pi)
                            Tubp->angle[3] = Tubp->angle[3] + 2 * pi * ((int)(Tubp->angle[3] / pi / 2));
                        Set_Tub(Tubp, Tubp->r, Tubp->angle, Tubp->xi);
                    }
                }
            ...
            if (numprocs != 1)
            {
                ...
                if (t % SlowTime == 0 && N > Npre)
                {
                    ...
                    for (i = 0; i < 13; i++)
                    {
                        if (Maxline < Tub[Top[i]].LineNo)
                            Maxline = Tub[Top[i]].LineNo;
                    }
                    if (Maxline > LCompute)
                    {
                    ...
                    if (mincom % numprocs)
                        MpiSec = mincom / numprocs + 1;
                    else
                        MpiSec = mincom / numprocs;
#else
                    if (N % numprocs)
                        MpiSec = N / numprocs + 1;
                    else
...
void output(int M)
{
    ...
    if (myid == 0)
    {
        ...
        if (M == 0)
            fvmd = fopen(filename, "wt+");
        else
            fvmd = fopen(filename, "at+");
        if (fvmd == NULL)
        {
        ...
        if (M == 0)
            frf = fopen(filename, "wt+");
        else
            frf = fopen(filename, "at+");
        if (frf == NULL)
        {
        ...
        for (j = 0; j < N; j++)
        {
            ...
            if (Tubp->Active)
            {
                ...
                for (k = 0; k < Tubp->AddNum; k++)
                {
                    ...
                    case 0x30:
                    case 0x40:
                        Tubp->PotenB += Tubp->PotenList[k];
                        break;
                    case 0x20:
                    case 0x50:
                    case 0x60:
                        Tubp->PotenA += Tubp->PotenList[k];
                        break;
                    }
                }
                Tubp->PotenA *= 0.5;
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1 && !strcmp(SRestart, argv[1]))
        ReInit(argv[2]);
    //    MPI_Barrier(MPI_COMM_WORLD);
    else
        init();
    ...
    if (!myid)
        ...
        {
        case 1:
            fprintf(stderr, "Finished because of convergence.\n");
            break;
        case 2:
            fprintf(stderr, "Finished at the critical point.\n");
            break;
        default:
            break;
        }
